...
if (isset($_GET['qno'])) {
	...
	if (is_numeric($qno)) {
		$query = "SELECT * FROM htmlquestions WHERE qno = '$qno' ";
		...
		if (mysqli_num_rows($run) > 0) {
			...
			window.location.href = 'allhtml.php'; </script>" ;
		}
	}
	else {
...
if(isset($_POST['submit'])) {
	...
	$query = "UPDATE htmlquestions SET question = '$question' , ans1 = '$choice1' , ans2= '$choice2' , ans3 = '$choice3' , ans4 = '$choice4' , correct_answer = '$correct_answer' WHERE qno = '$qno' ";
	$run = mysqli_query($conn , $query) or die(mysqli_error($conn));
